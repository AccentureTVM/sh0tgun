...
def main(args):
	...
	port = args[2].strip()
	root = args[3]
	logger.info("Performing nmap FTP script scan for " + ip_address + ":" + port)
	...
	try:
		...
		logger.error("FTP NSE scan failed for " + ip_address + ":" + port)
		return
	try:
		...
		for line in lines:
			if "Anonymous FTP login allowed" in line:
				logger.warning("Anonymous FTP Login on " + ip_address) 
		...
		logger.error("FTP NSE scan succeeded, however post processing has failed for " + ip_address + ":" + port + "\n\t\tSee raw nmap scan results in " + root + "discovery/ftp/")
