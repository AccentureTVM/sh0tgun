...
def mssqlCrack(ip_address, port, root):
	MEDUSA = "medusa -h %s -U %s -P %s -v6 -n %s -e ns -M mssql > %sdiscovery/mssql/medusa_%s.txt" % (ip_address, "/root/wordlists/admin_usernames.txt", "/root/wordlists/pw.txt", port, root, ip_address)	
	results = subprocess.check_output(MEDUSA, shell=True)
	results = results.decode('utf-8')
	results = results.split("\n")
	fo = open(root + "password/passwords.csv", 'w+')
